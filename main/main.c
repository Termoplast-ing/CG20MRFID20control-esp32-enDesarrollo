...
void app_main(void) {
    ...
    esp_err_t ret = modbus_write_single_register(5, 0x0010, 0x1234);
    if (ret == ESP_OK) {
        printf("Registro escrito correctamente.\n");
    } else {
        printf("Error escribiendo el registro.\n");
    }
    ...
    vTaskDelay(pdMS_TO_TICKS(1000));
        ESP_LOGI(TAG, "🔍 Enviando solicitud 0x03");
        modbus_send_read_holding_registers(5, 0x0000, 1);

        int len = modbus_receive_response(buffer, sizeof(buffer));
        if (len > 0) {
        ...
        vTaskDelay(pdMS_TO_TICKS(1000));
        ESP_LOGI(TAG, "🔍 Enviando solicitud 0x06");
        ret = modbus_write_single_register(5, 0x0010, 0x1234);
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "✅ Registro escrito correctamente");
        } else {
            ESP_LOGW(TAG, "⚠️ Error escribiendo el registro");
        }
        vTaskDelay(pdMS_TO_TICKS(1000));
        ESP_LOGI(TAG, "🔍 Enviando solicitud 0x10");
        ret = modbus_write_multiple_registers(5, 0x0000, 2, (const uint16_t[]){0x1234, 0x5678});
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "✅ Registros escritos correctamente");
        } else {
            ESP_LOGW(TAG, "⚠️ Error escribiendo los registros");
        }
...
void app_main(void)
{
    ...
    uint16_t values_to_write[] = {0x1111, 0x2222, 0x3333};
    uint16_t quantity = sizeof(values_to_write) / sizeof(values_to_write[0]);

    while (1) {
        printf("📤 Enviando función 0x10 a esclavo %d...\n", SLAVE_ID);

        esp_err_t result = modbus_write_multiple_registers(
            SLAVE_ID, START_ADDRESS, quantity, values_to_write
        );

        if (result == ESP_OK) {
            printf("✅ Registro escrito correctamente!\n");
        } else {
            printf("❌ Error escribiendo registros: 0x%x\n", result);
        }
